...
int syscall(int syscall_nr, ...)
{
  ...
  switch(syscall_nr)
  {
    case SYS__assert:
    {
      ...
      _assert(filename, linenum, msg, reg);
      break;
    }
    ...
    {
      mqd_t mqdes = va_arg(ap, mqd_t);
      const struct sigevent *notification = va_arg(ap, const struct sigevent *);
    ...
    case SYS_posix_spawn:
    {
      pid_t *pid = va_arg(ap, pid_t *);
      const char *path = va_arg(ap, const char *);
      const posix_spawn_file_actions_t *file_actions = va_arg(ap, const posix_spawn_file_actions_t *);
    ...
    case SYS_timer_settime:
    {
      int clockid = va_arg(ap, int);
      int flags = va_arg(ap, int);
      const struct itimerspec *new_value = va_arg(ap, const struct itimerspec *);
