...
*
* The steps are:
* -> register a handler for SIGPIPE without SA_SIGINFO, and a known function
*   as sa_handler
* -> raise SIGPIPE, and check the function has been called.
*
...
int main(void)
{
	...
	if (ret != 0) {
		perror("Failed to raise SIGPIPE");
		return PTS_UNRESOLVED;
	}
