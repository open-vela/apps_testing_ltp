...
*
* The steps are:
* -> register a handler for SIGTSTP without SA_SIGINFO, and a known function
*   as sa_handler
* -> raise SIGTSTP, and check the function has been called.
*
...
int main(void)
{
	...
	if (ret != 0) {
		perror("Failed to raise SIGTSTP");
		return PTS_UNRESOLVED;
	}
