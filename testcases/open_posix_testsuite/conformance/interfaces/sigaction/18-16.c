...
*
* The steps are:
* -> register a handler for SIGTTOU without SA_SIGINFO, and a known function
*   as sa_handler
* -> raise SIGTTOU, and check the function has been called.
*
...
int main(void)
{
	...
	if (ret != 0) {
		perror("Failed to raise SIGTTOU");
		return PTS_UNRESOLVED;
	}
